...
export class PoseActivator {
  ...
  public initWebcamButton = async (iMarkable: IMarkable) => {
    ...
    return new Promise<void>(async (resolve) => {
      ...
      this.webcamButton.addEventListener("click", async () => {   
        ...
        await WebcamActivator.enableCam(iMarkable, new CanvasRenderer(this.videoElement));
  ...
  private initializeHTMLElements = (outputDiv: HTMLDivElement) => {
    ...
    // creates the HTML elements 
      this.videoElement = ElementInitializer.createVideoElement(outputDiv, "webcam");      
      this.canvas = ElementInitializer.createCanvasElement(outputDiv, "output_video");
      this.webcamButton = ElementInitializer.createButtonElement(outputDiv, "enableWebcamButton");
